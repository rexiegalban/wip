			...
			<span style="margin-left:15px;">
				...
				<small> PACKAGE:<span style="color:#428BCA;"> <b><?php echo @$_REQUEST['pkg']; ?></b> </span></small>
					...
					<div class="panel-body">
						...
						<script>$(document).ready(function() { $('#missed-sod-device').DataTable({"lengthMenu": [[15, 50, 100, -1], [15, 50, 100, "All"]],	"order": [[ 7, "desc" ]]}); } );</script>

						<table id="missed-sod-device" class="table table-striped table-bordered" cellspacing="0" width="100%">
							<thead>
								<tr>
							...
							<tbody>
								...
								$device = new MissedSodController($_REQUEST);
								if($data = $device->getMissedSodDevice()){
									foreach($data as $row){
