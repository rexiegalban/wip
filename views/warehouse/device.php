			...
			<span style="margin-left:15px;">
				...
				<small> PACKAGE:<span style="color:#428BCA;"> <b><?php echo @$_REQUEST['pkg']; ?></b> </span></small>
					...
					<div class="panel-body">
						...
						<script>$(document).ready(function() { $('#warehouse-device').DataTable({"lengthMenu": [[15, 50, 100, -1], [15, 50, 100, "All"]],	"order": [[ 7, "desc" ]]}); } );</script>

						<table id="warehouse-device" class="table table-striped table-bordered" cellspacing="0" width="100%">
							<thead>
								<tr>
							...
							<tbody>
								...
								$device = new WarehouseController($_REQUEST);
								if($data = $device->getWarehouseDevice()){
									foreach($data as $row){
