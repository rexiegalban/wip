...
header("Content-Disposition: attachment; filename=active-mo-device.xls");
 //header
echo "DEVICE\t# of ACTIVE MO\tSTART\t RUNNING\tQUEUE\tCOMPLETED\tYIELD %\tMO INV VALUE in $\tMTL USAGE COST in $\t\n";
//content
require ('controllers/activeMOController.php'); 
$packages = new ActiveMOController($_REQUEST);
if($data = $packages->getActiveMoSaveXlsDevice()){
    foreach($data as $row){
