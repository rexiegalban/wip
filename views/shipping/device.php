			...
			<span style="margin-left:15px;">
				...
				<small> PACKAGE:<span style="color:#428BCA;"> <b><?php echo @$_REQUEST['pkg']; ?></b> </span></small>
					...
					<div class="panel-body">
						...
						<script>$(document).ready(function() { $('#shipping-device').DataTable({"lengthMenu": [[15, 50, 100, -1], [15, 50, 100, "All"]],	"order": [[ 5, "desc" ]]}); } );</script>

						<table id="shipping-device" class="table table-striped table-bordered" cellspacing="0" width="100%">
							<thead>
								<tr>
							...
							<tbody>
								...
								$device = new ShippingController($_REQUEST);
								if($data = $device->getShippingDevice()){
									foreach($data as $row){
